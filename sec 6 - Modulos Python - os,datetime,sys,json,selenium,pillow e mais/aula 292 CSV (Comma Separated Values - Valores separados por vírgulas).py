...
# Define o caminho do arquivo CSV usando a biblioteca pathlib.
CAMINHO_CSV = Path(__file__).parent / 'aula 292.csv'

# Abre o arquivo CSV em modo de leitura ('r') usando um gerenciador de contexto (with).
with open(CAMINHO_CSV, 'r') as arquivo:
    # Cria um leitor CSV que lê o arquivo como um dicionário, onde as chaves são os cabeçalhos das colunas.
    leitor = csv.DictReader(arquivo)

    # Itera sobre as linhas do arquivo CSV e imprime o Nome, Idade e Endereço de cada linha.
    for linha in leitor:
        print(linha['Nome'], linha['Idade'], linha['Endereço'])

# Lista de dicionários que representa os clientes a serem escritos no arquivo CSV.
lista_clientes = [
...
# Abre o arquivo CSV em modo de escrita ('w') usando um gerenciador de contexto (with).
with open(CAMINHO_CSV, 'w') as arquivo:
    # Obtém os nomes das colunas a partir do primeiro dicionário na lista de clientes.
    nome_colunas = lista_clientes[0].keys()

    # Cria um escritor CSV que escreverá os dados como um dicionário, usando os nomes das colunas como cabeçalhos.
    ...
    # Itera sobre a lista de clientes e escreve cada cliente no arquivo CSV.
